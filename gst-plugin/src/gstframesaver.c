...
static GstFlowReturn gst_frame_saver_chain(GstPad *pad, GstObject *parent, GstBuffer *buffer) {
    ...
    // Forward the buffer to the single queue for downstream processing
...
static void gst_frame_saver_init(GstFrameSaver *saver) {
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_type_class_peek(GST_TYPE_FRAME_SAVER));

    // Replace the existing element class with GstBinClass
    g_type_class_adjust_private_offset(element_class, (gint)sizeof(GstBinClass));

    // Create a bin instead of a simple element
    saver->bin = gst_bin_new("framesaver_bin");
    gst_element_set_state(GST_ELEMENT(saver->bin), GST_STATE_NULL);

    // Add the bin to the element
    gst_bin_add(GST_BIN(saver), saver->bin);

    // Create a single queue for buffering processed frames
    saver->queue = gst_element_factory_make("queue", "output_queue");
    gst_bin_add(GST_BIN(saver->bin), saver->queue);

    // Create tee element for parallel processing
    saver->tee = gst_element_factory_make("tee", "tee");
    gst_bin_add(GST_BIN(saver->bin), saver->tee);

    // Link elements: tee -> queue -> sinkpad
    gst_element_link_many(saver->tee, saver->queue, saver->sinkpad, NULL);

    // Link tee pads to the queue
    GstPad *tee_pad = gst_element_get_request_pad(saver->tee, "src_%u");
    GstPad *queue_pad = gst_element_get_static_pad(saver->queue, "sink");
